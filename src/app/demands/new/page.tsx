              ...
              <ScrollArea className="flex-1 w-full">
                <div className="min-w-[1400px]">
                  <Table>
                    ...
                    <TableBody>
                      {rows.map((row, index) => (
                        <TableRow key={row.id} className={cn("transition-all group border-b border-border/40 bg-background hover:bg-accent/5", index === rows.length - 1 && "border-l-[4px] border-l-primary")}>
                          <TableCell className="p-1 px-3">
                            <Select value={row.unitName} onValueChange={(val) => updateRow(row.id, { unitName: val, sector: "" })}>
                          ...
                          <TableCell className="p-1 px-3">
                            <Input placeholder="Setor" className="border-none shadow-none bg-transparent h-9 text-xs" value={row.sector} onChange={(e) => updateRow(row.id, { sector: e.target.value })} />
                          </TableCell>
                          <TableCell className="p-1 px-3">
                          ...
                          <TableCell className="p-1 px-3">
                            <Input placeholder="O que aconteceu?" className="border-none shadow-none bg-transparent h-9 text-xs italic" value={row.freeText} onChange={(e) => updateRow(row.id, { freeText: e.target.value })} />
                          </TableCell>
                          <TableCell className="p-1 px-3">
                            <Input placeholder="O que você fez?" className="border-none shadow-none bg-transparent h-9 text-xs" value={row.details} onChange={(e) => updateRow(row.id, { details: e.target.value })} />
                          </TableCell>
                          <TableCell className="p-1 px-3 text-center">
                            {index !== rows.length - 1 && <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full" onClick={() => removeRow(row.id)}><Trash2 className="w-3.5 h-3.5" /></Button>}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
